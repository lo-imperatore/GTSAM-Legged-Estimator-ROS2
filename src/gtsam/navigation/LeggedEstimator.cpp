...
std::shared_ptr<PreintegrationCombinedParams> combinedPreintegrationParams(
  ...
  if (!combinedParams) {
    ...
    combinedParams->setUse2ndOrderCoriolis(
        params.preintegrationParams->getUse2ndOrderCoriolis());
...
NavState fullContactInitializationNavState(
  ...
  Rot3 attitude = seedNavState.attitude();
  if (activeContacts.size() >= 3) {
    ...
    Vector3 normal =
        Eigen::JacobiSVD<Matrix>(centered, Eigen::ComputeFullU)
            .matrixU()
            .col(2);
    if (normal.z() < 0.0) {
      normal = -normal;
    ...
    attitude = Rot3::Ypr(seedRpy.z(), pitch, roll);
  }
...
void LeggedFixedLagSmoother::processContacts(
  ...
  if (hasPendingImu()) {
    ...
    factors.emplace_shared<LeggedImuFactor2>(previousBaseKey, baseKey,
                                             MakeBiasKey(), pim_);
...
void LeggedCombinedFixedLagSmoother::processContacts(
  ...
  if (hasPendingImu()) {
    ...
    baseState = deadReckonedState_;
    factors.emplace_shared<LeggedCombinedImuFactor>(
