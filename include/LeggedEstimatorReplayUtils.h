...
/// One buffered trajectory sample (written to CSV/TUM at the end of replay).
struct ReplayTrajectoryRow {
  std::string kind;
  double timestampS = 0.0;
  NavState state;
};
...
struct ReplayOutputs {
  ...
  NavState finalState;
  std::vector<ReplayTrajectoryRow> trajectoryRows;
};
...
inline std::vector<ContactEvent> loadContactEvents(const fs::path& path) {
  ...
  for (size_t rowIndex = 1; rowIndex < rows.size(); ++rowIndex) {
    ...
    if (fields.size() == 7) {
      // New compact schema:
      // event_index,timestamp_s,foot_index,body_x,body_y,body_z,is_new_contact
      measurement.touchdown = parseSize(fields[6], "is_new_contact") != 0;
      measurement.bodyPoint = Vector3(parseDouble(fields[3], "body_x"),
                                      parseDouble(fields[4], "body_y"),
                                      parseDouble(fields[5], "body_z"));
    } else {
      // Backward-compatible schema with foot_name:
      // event_index,timestamp_s,foot_index,foot_name,is_new_contact,body_x,body_y,body_z
...
/// Write one TUM trajectory row (IMU frame pose).
/// Format: timestamp tx ty tz qx qy qz qw
inline void writeTumTrajectoryRow(std::ofstream& output, double timestampS,
                                  const NavState& state) {
  const auto quaternion = state.quaternion();
  output << toString(timestampS) << ' ' << toString(state.position().x()) << ' '
         << toString(state.position().y()) << ' '
         << toString(state.position().z()) << ' ' << toString(quaternion.x())
         << ' ' << toString(quaternion.y()) << ' ' << toString(quaternion.z())
