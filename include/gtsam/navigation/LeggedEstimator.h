...
#ifdef GTSAM_LEGGED_USE_GALILEAN_IMU
...
#ifdef GTSAM_LEGGED_USE_GALILEAN_IMU
using LeggedPreintegratedImuMeasurements = PreintegratedImuMeasurementsG;
using LeggedImuFactor2 = GalileanImuFactor2;
using LeggedPreintegratedCombinedMeasurements =
    PreintegratedCombinedMeasurementsG;
using LeggedCombinedImuFactor = GalileanCombinedImuFactor;
#else
using LeggedPreintegratedImuMeasurements = PreintegratedImuMeasurements;
using LeggedImuFactor2 = ImuFactor2;
using LeggedPreintegratedCombinedMeasurements =
    PreintegratedCombinedMeasurements;
using LeggedCombinedImuFactor = CombinedImuFactor;
#endif
...
struct LeggedEstimatorParams {
  ...
  Pose3 body_P_imu = Pose3(Rot3(), Point3(0.0, 0.0, 0.0));
  double footholdProcessSigma = 1e-3;
  double footholdInitSigma = 5e-1;
  Matrix3 contactCovariance =
      (Vector3(0.01 * 0.01, 0.01 * 0.01, 0.01 * 0.01)).asDiagonal();
...
class GTSAM_EXPORT LeggedFixedLagSmoother : public LeggedEstimator {
 ...
 private:
  ...
  LeggedPreintegratedImuMeasurements pim_;
  size_t step_ = 0;
...
class GTSAM_EXPORT LeggedCombinedFixedLagSmoother : public LeggedEstimator {
 ...
 private:
  ...
  Matrix9 baseCovariance0_;
  BatchFixedLagSmoother smoother_;
  LeggedPreintegratedCombinedMeasurements pim_;
  size_t step_ = 0;
  double currentTime_ = 0.0;
  std::vector<bool> inContact_;
  std::vector<bool> initialized_;
